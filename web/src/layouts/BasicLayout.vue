  ...
  <div class="app">
      ...
      <div>
        ...
        <div class="nav">
          ...
          <router-link to="/overview" :class="{ active: isActive('/overview') }">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg>
            总览
          </router-link>
          ...
          <router-link to="/alerts" :class="{ active: isActive('/alerts') }">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>
            告警中心
          </router-link>

          <router-link to="/reports" :class="{ active: isActive('/reports') }">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M3 3v18h18"/><path d="M7 16l4-8 4 4 4-6"/></svg>
            报表
          </router-link>

          <router-link to="/warehouse3D" :class="{ active: isActive('/warehouse3D') }">
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M3 7.5 12 3l9 4.5v9L12 21l-9-4.5v-9Z"/><path d="M12 21v-9"/><path d="M21 7.5 12 12 3 7.5"/></svg>
            3D仓库
          </router-link>
    ...
    <div style="display: flex; flex-direction: column; flex: 1; min-width: 0;">
      ...
      >
        ...
        ></span>
        <template v-if="wsFallbackMode">WebSocket 连接断开，已切换到受限模式</template>
        <template v-else>
          正在重新连接实时数据流... ({{ wsRetryCount }}/3)
          <span v-if="wsNextRetryIn > 0" style="margin-left: 8px; opacity: 0.8; font-weight: 500;">
            {{ wsNextRetryIn }}s 后重试
          </span>
        </template>
      </div>
    ...
    <div v-if="shortcutHelpVisible" class="shortcut-help glass">
      <h4>键盘快捷键</h4>
      <ul>
        <li><kbd>?</kbd><span>显示或隐藏</span></li>
        <li><kbd>1</kbd><span>总览</span></li>
        <li><kbd>2</kbd><span>摄像头</span></li>
        <li><kbd>3</kbd><span>告警中心</span></li>
        <li><kbd>4</kbd><span>报表</span></li>
        <li><kbd>5</kbd><span>3D 仓库</span></li>
        <li><kbd>6</kbd><span>模型管理</span></li>
        <li><kbd>7</kbd><span>系统设置</span></li>
...
.brand-mark {
  ...
  box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.18), inset 0 -1px 2px rgba(0, 0, 0, 0.4), 0 3px 8px rgba(10, 10, 15, 0.18);
}
