...
- 安全/审计要求：`RetrainingConfig.require_human_confirmation` 默认为 `True`，调度器创建的 backbone 任务会以 `pending_confirmation` 状态等待人工放行（见 `src/argus/core/scheduler.py` 中 `create_backbone_retraining_task` 的 `_backbone_check` 内层函数）。
- 资源占用：训练阶段会主动把 YOLO 等推理模型迁出 GPU（`ModelTrainer._free_gpu_for_training`，见 `src/argus/anomaly/trainer.py`），训练期间 GPU 推理会受影响。
- 数据质量风险：基线/标注质量不达标时，自动激活的新模型可能反向劣化检测效果。

**何时决定重训练**（实现见 `create_retraining_task`，位于 `src/argus/core/scheduler.py`）：
...
4. 训练完成后若 `auto_deploy=True` 且质量等级 `≥ auto_deploy_min_grade`，调用 `ModelRegistry.activate(version_id, triggered_by="auto_retrain")`。注：`allow_bypass=True` 已在 commit `91dfc2f`（2026-04 释放管线安全收紧）中移除，自动激活同样需走 stage gate 校验，未达 PRODUCTION 资格的版本会留作 CANDIDATE 等待人工。
...
- 注册位置：`create_retraining_task`（位于 `src/argus/core/scheduler.py`）。
...
- 注册位置：`create_backbone_retraining_task`（位于 `src/argus/core/scheduler.py`）。
- 周期：固定 24h 检查一次。
...
- 调度器另注册 `create_job_processing_task`（位于 `src/argus/core/scheduler.py`）每分钟扫一次 `training_jobs` 表里 `status=QUEUED` 的任务，由 `TrainingJobExecutor.process_queued_jobs()` 执行。
- 触发来源：
  - `manual`：dashboard 训练任务弹窗。
  - `drift_suggested`：标注队列里漂移规则触发，写入位置见 `src/argus/dashboard/routes/labeling.py` 的漂移建议处理函数。
  - `scheduled`：上述 backbone 检查产生。
- 注意：该队列处理任务无论 `retraining.enabled` 是否打开都会注册（参见 `src/argus/__main__.py` 中 `_register_training_job_processing` 的调用），它消费的是“已经在数据库里 `QUEUED`”的任务。`retraining.enabled` 只影响 2.1 / 2.2 两个 scheduled 入口。
...
- 全部读取自 `BaselineManager`：
  - 单摄像头：`baselines_dir/{camera_id}/default/v{NNN}/baseline_*.png|jpg`（`current.txt` 标记当前版本，见 `src/argus/anomaly/baseline.py` 的版本目录解析段）。
  - 摄像机组：`baselines_dir/groups/{group_id}/{zone_id}/`（无版本子目录）。
- **生命周期 gate**：`ModelTrainer.train` 会检查 baseline 是否处于 `Verified` 或 `Active` 状态，否则训练直接失败（见 `src/argus/anomaly/trainer.py` 的生命周期校验段）。
...
- **不依赖**：当前 `create_retraining_task` 只看磁盘上 baseline 图片数量，与 `ActiveLearningSampler` 的 `RetrainingTriggered` 事件**没有订阅链路**（grep `RetrainingTriggered` 结果只有发布者本身，见 `src/argus/core/active_learning.py`）。
...
- **训练侧硬下限**：`MIN_BASELINE_IMAGES = 30`（常量定义见 `src/argus/anomaly/trainer.py`）。低于此数量，`ModelTrainer.train` 会走 few-shot 分支或直接 `FAILED`。
- **触发下限**：`retraining.min_new_baselines`（默认 20，范围 5–500）。建议保持 ≥ 30，与训练侧硬下限对齐。
- **真实标签评估下限**：`val_precision/recall/f1/auroc/pr_auc` 仅在 `data/validation/{cam}/confirmed` 与 `data/baselines/{cam}/false_positives` 各自 ≥10 张时才会写入（见 `src/argus/storage/models.py` 的真实标签评估字段计算）。否则训练记录里这些字段为空，质量等级仅基于 normal-holdout 的 `val_score_*`。

---

## 4. 配置项

来自 `src/argus/config/schema.py` 中的 `RetrainingConfig`，对应 `configs/default.yaml` 的 `retraining:` 段。
...
| `auto_deploy_min_grade` | `B` | A/B/C/F | 自动激活的最低质量等级。等级排序：A=4 > B=3 > C=2 > F=1（见 `src/argus/core/scheduler.py` 中 `_GRADE_RANK`）。|
...
1. **确认前置依赖**
   - `pip install apscheduler` 已安装（缺失则 `src/argus/core/scheduler.py` 的导入容错段会打 `scheduler.apscheduler_not_available` 警告并放弃所有计划任务）。
...
- **不暂停**，但训练期间 GPU 推理性能会下降：
  - `ModelTrainer._free_gpu_for_training()` 把所有 `_shared_yolo_registry` 中的 YOLO 模型 `.to("cpu")`，并 `torch.cuda.empty_cache()`（见 `src/argus/anomaly/trainer.py` 的 `_free_gpu_for_training` 函数）。
  - 训练完 `_restore_gpu_after_training()` 把它们再迁回 `cuda:0`（同文件 `_restore_gpu_after_training` 函数）。
- **训练完成后是热加载，不需要重启**：`register_training_job_processing` 注入了 `_on_model_trained` 回调，调用 `pipeline.reload_anomaly_model(model_path)`（见 `src/argus/runtime/training_job_wiring.py` 的回调注册段）。回调在 daemon 线程里跑（见 `src/argus/anomaly/job_executor.py` 的回调线程段）。注意 commit `a68db43`（2026-04 释放管线收紧）后训练阶段不再热载候选模型，新版本仅在通过 release pipeline 后才会被采用。
...
- **训练失败**：`TrainingResult.status != COMPLETE` 时，`create_retraining_task` 内层只记 `retraining.camera_failed` 日志并继续下一个摄像头，**不会自动回退**当前激活模型。
- **打包失败**：`TrainingJobExecutor._execute_head` 会把刚注册的 CANDIDATE 通过 `ModelRegistry.retire()` 回收，避免 ghost 版本（见 `src/argus/anomaly/job_executor.py` 的 `_execute_head` 函数）。
- **质量不达标**：`auto_deploy=true` 但 `grade < auto_deploy_min_grade` 时，仅记 `retraining.skip_deploy` 日志，旧模型保持 active。
- **stale RUNNING 任务**：`TrainingJobExecutor.recover_stale_jobs(max_running_hours=6.0)` 每分钟扫描一次，把 RUNNING 超过 6h 的任务标记 FAILED（同文件 `recover_stale_jobs` 函数）。
- **手动回退**：`ModelRegistry.rollback(camera_id)` 可以回到上一个 PRODUCTION 版本（见 `src/argus/storage/model_registry.py` 的 `rollback` 函数）。需运维主动调用，无自动 rollback 逻辑。

### 6.3 模型版本管理

- 自动激活路径：`create_retraining_task` 内的 `_retraining_check` 调 `ModelRegistry.register(...)` 拿到 `version_id`，再调用 `activate(version_id, triggered_by="auto_retrain")`。**注意：** commit `91dfc2f`（2026-04 安全收紧）已移除 `allow_bypass=True`，自动激活同样要走 release pipeline 的 stage gate（见 `src/argus/storage/model_registry.py` 的 `activate` 函数），未达 PRODUCTION 资格的版本会留作 CANDIDATE 等待人工。
...
- **Models 页 → 训练与评估 tab**（`web/src/views/Models.vue` 中的训练 tab 段）：
...
**Q4：自动激活的版本质量不行，怎么回退？**
- 调用 `ModelRegistry.rollback(camera_id, triggered_by="ops")`（见 `src/argus/storage/model_registry.py` 的 `rollback` 函数）回到上一 PRODUCTION 版本；或在 Models 页“模型管理”里手动激活历史版本。当前**没有自动 rollback 逻辑**。
