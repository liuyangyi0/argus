...
| RBAC 用户权限 | admin/operator/viewer 三级角色，会话管理 |
| 审计日志 | 操作审计表（用户/动作/目标/详情/IP） |
| 统计报表 | 日报/周报、趋势分析 |
| 数据备份恢复 | SQLite 在线备份、配置回滚 |
| 键盘快捷键 | j/k 导航、a 确认、f 标记误报、? 帮助 |
| 浏览器通知 | Notification API 推送 |
| WebSocket 实时推送 | 替代 HTMX 轮询，主题订阅 |
...
详见 [upgrade-tasks.md](upgrade-tasks.md)，57 个子任务分 4 阶段：

- **Phase A**: CUSUM 时序累积 / Conformal 校准 / Simplex 双通道 / 主动学习基线
- **Phase B**: Dinomaly2 骨干集成 / INT8 量化导出
- **Phase C**: 摄像头健康层 / KS 漂移监控 / 跨摄像头关联 / 最小 MLOps
- **Phase D**: YOLO-World OVD / SAM 2 分割 / 合成异常数据
