...
def main():
    ...
    go2rtc_instance = getattr(app, "state", None) and getattr(app.state, "go2rtc", None) if app else None
    if go2rtc_instance is None:
        # go2rtc may not be wired into app.state yet; check dashboard config
