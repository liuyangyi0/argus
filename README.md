...
`smoke_physical_scenarios.py` 是上述语义断言的现场包装入口，默认使用 OBSBOT `1920x1080@60` + go2rtc + 浏览器检查，并在现场动作前先跑同配置 preflight；运行后按提示在 `activation-delay` 内放书或让小物体快速经过画面。调试时可用 `--no-preflight` 跳过预检。
