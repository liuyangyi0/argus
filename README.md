...
# 真实物理场景验收：桌面放书应是 scene_change/静态异物，小物体飞过应是 projectile
python scripts/smoke_dashboard_business_flow.py --camera-source 0 --camera-protocol usb --camera-resolution 1920,1080 --usb-device-name "OBSBOT Meet 2 StreamCamera" --require-go2rtc --activation-delay 10 --expect-alert-category scene_change --expect-detection-type anomaly --forbid-alert-category projectile --forbid-detection-type projectile --browser required
python scripts/smoke_dashboard_business_flow.py --camera-source 0 --camera-protocol usb --camera-resolution 1920,1080 --usb-device-name "OBSBOT Meet 2 StreamCamera" --require-go2rtc --activation-delay 10 --expect-alert-category projectile --expect-detection-type projectile --browser required
