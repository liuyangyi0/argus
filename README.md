...
`smoke_physical_scenarios.py` 是上述语义断言的现场包装入口，默认使用 OBSBOT `1920x1080@60` + go2rtc + 浏览器检查，并在现场动作前先跑同配置 preflight；`stable` 会保持画面不变并要求观察窗口内无告警，`book/projectile` 会按提示在 `activation-delay` 内放书或让小物体快速经过画面。最终 JSON 会提炼 preflight 的实测 FPS/go2rtc 状态和场景告警语义，便于现场验收留档；需要先确认硬件链路但暂不做人工动作时可用 `--preflight-only`；需要保留数据库、截图、热力图和 Replay 证据时传 `--work-dir <dir> --keep-work-dir`，脚本会为 preflight/stable/book/projectile 分配独立子目录，并把总结果写到 `<work-dir>/physical_smoke_summary.json`；需要指定总结果位置时可传 `--summary-path <file>`；调试时可用 `--no-preflight` 跳过预检。
