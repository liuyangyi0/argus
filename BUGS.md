...
| console.log/warn/error | 23 处 | **0** (生产环境静默) |
| TypeScript `: any` | 134 处 | **115** (-19, 新增 7 个接口类型) |
| 内联 style | 400+ 处 | **大幅减少** (VideoTile 18→0, Alerts 22→0) |
...
### 浏览器验证通过
- [x] WebSocket 实时推送 — 健康数据实时更新，无断连横幅
- [x] 摄像头实时流 — MJPEG 回退正常，go2rtc 流协商工作
- [x] 告警回放播放器 — 热力图叠加、帧控制、信号轨道正常
- [x] 区域编辑器 — 快照底图加载、添加区域按钮就绪
- [x] 训练任务创建 — 任务列表、状态筛选、详情按钮正常
- [x] 基线采集 — 基线表格、版本管理、快速/高级采集按钮正常
